...
def gen_bert_labels(q_seq, q_lens, sel_col_nums, where_col_nums, ans_seq, gt_cond_seq, header_type, col_seq):
	...
	return where_conn_label, sel_num_label, where_num_label, sel_col_label, sel_agg_label, where_col_label, where_op_label, where_start_label, where_end_label
...
def epoch_train(model, optimizer, batch_size, sql_data, table_data, tokenizer=None):
	...
	for st in tqdm(range(len(sql_data) // batch_size + 1)):
		...
		if isinstance(model, SQLBert):
			...
			bert_inputs, q_lens, sel_col_nums, where_col_nums = gen_batch_bert_seq(tokenizer,
																				   q_seq,
																				   col_seq,
																				   header_type)
...
def predict_test(model, batch_size, sql_data, table_data, output_path, tokenizer=None):
	...
	for st in tqdm(range(len(sql_data) // batch_size + 1)):
		...
		with torch.no_grad():
			if isinstance(model, SQLBert):
				q_seq, col_seq, col_num, raw_q_seq, table_ids, header_type = to_batch_seq_test(sql_data,
																							   table_data,
																							   perm,
																							   st,
																							   ed,
...
def predict_predict(model, batch_size, sql_data, table_data, tokenizer=None):
	model.eval()
	perm = list(range(len(sql_data)))
	result = []
	for st in tqdm(range(len(sql_data) // batch_size + 1)):
