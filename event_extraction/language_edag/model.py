...
import numpy as np
from event_model import EventTable, SentencePosEncoder, MentionTypeEncoder, AttentiveReducer
from ner_model import LanguageForBasicNERModel
import transformer
...
class Doc2EDAGModel(BertPreTrainedModel):
    ...
    def __init__(self, args):
        ...
        # self.ner_model = LanguageForBasicNERModel.from_pretrained(args)
        self.ner_model = LanguageForBasicNERModel(args)
        ...
        class PseudoPooler(object):
            ...
            def __call__(self, *x):
                return x
        del self.ner_model.bert.pooler
        self.ner_model.bert.pooler = PseudoPooler()

        # all event tables
        self.event_tables = nn.ModuleList([
            EventTable(event_type, field_types, args.hidden_size)
        ...
        self.sent_pos_encoder = SentencePosEncoder(
            args.hidden_size, max_sent_num=args.max_sent_num, dropout=args.dropout
        )

        if self.args.use_token_role:
            self.ment_type_encoder = MentionTypeEncoder(
                args.hidden_size, args.num_entity_labels, dropout=args.dropout
            )

        # various attentive reducer

        if self.args.seq_reduce_type == 'AWA':
            self.doc_token_reducer = AttentiveReducer(args.hidden_size, dropout=args.dropout)
            self.span_token_reducer = AttentiveReducer(args.hidden_size, dropout=args.dropout)
            self.span_mention_reducer = AttentiveReducer(args.hidden_size, dropout=args.dropout)
        else:
            assert self.args.seq_reduce_type in ['MaxPooling', 'MeanPooling']

        if self.args.use_doc_enc:
            self.doc_context_encoder = transformer.make_transformer_encoder(
                args.num_tf_layers, args.hidden_size, ff_size=args.ff_size, dropout=args.dropout
            )

        if self.args.use_path_mem:
            self.field_context_encoder = transformer.make_transformer_encoder(
                args.num_tf_layers, args.hidden_size, ff_size=args.ff_size, dropout=args.dropout
            )

    def adjust_token_label(self, doc_token_labels_list):
        if self.args.use_token_role:  # do not use detailed token
    ...
    def get_batch_sent_emb(self, ner_token_emb, ner_token_masks, valid_sent_num_list):
        ...
        elif self.args.seq_reduce_type == 'MaxPooling':
            total_sent_emb = ner_token_emb.max(dim=1)[0]
        elif self.args.seq_reduce_type == 'MeanPooling':
            total_sent_emb = ner_token_emb.mean(dim=1)
        else:
            raise Exception('Unknown seq_reduce_type {}'.format(self.args.seq_reduce_type))
    ...
    def get_local_context_info(self, doc_batch_dict, train_flag=False, use_gold_span=False):
        ...
        if train_flag and use_gold_span:
    ...
    def get_field_cls_info(self, event_idx, field_idx, batch_span_emb,
        ...
        if train_flag:
            ...
            class_weight = torch.tensor(
                [self.args.neg_field_loss_scaling, 1.0], device=device, dtype=data_type, requires_grad=False
            )
    ...
    def get_mix_loss(self, doc_sent_loss_list, doc_event_loss_list, doc_span_info_list):
        ...
        lambda_1 = self.args.loss_lambda
        lambda_2 = 1 - lambda_1
    ...
    def forward(self, doc_batch_dict, doc_features, use_gold_span=False, train_flag=True, teacher_prob=1,
        ...
        doc_token_emb_list, doc_token_masks_list, doc_token_types_list, doc_sent_emb_list, doc_sent_loss_list = \
            self.get_local_context_info(doc_batch_dict, train_flag=train_flag, use_gold_span=use_gold_span)
...
class DCFEEModel(BertPreTrainedModel):
    """
    ...
    def __init__(self, args):
        ...
        self.config_class, _, config_model = MODEL_CLASSES[args.pred_model_type]
        bert_config = self.config_class.from_pretrained(args.model_name_or_path)
        super(DCFEEModel, self).__init__(bert_config)
        ...
        self.event_cls_layers = nn.ModuleList([
            nn.Linear(args.hidden_size, 2) for _ in self.event_type_fields_pairs
        ])
    ...
    def get_mix_loss(self, doc_sent_loss_list, doc_event_loss_list, doc_span_info_list):
        ...
        for doc_sent_loss, doc_span_info in zip(doc_sent_loss_list, doc_span_info_list):
            ...
            sent_loss_scaling[doc_span_info.missed_sent_idx_list] = self.args.loss_gamma
