...
from model import EntityClassificationModel
import numpy as np
import torch
...
class Trainer:
    ...
    def __init__(self, args, train_dataset=None, dev_dataset=None, test_dataset=None):
        ...
        self.model_class = EntityClassificationModel
    ...
    def train(self):
        ...
        witer = SummaryWriter(logdir=self.args.model_dir, comment="intent")
    ...
    def evaluate(self, mode):
        ...
        if self.args.is_muti_label:
            ...
            out_intent_label_ids = out_intent_label_ids.tolist()
            intent_preds = intent_preds.tolist()

            intent_label_map = self.args.id_label

            for y1 in out_intent_label_ids:
                y12 = []
                for idx, y11 in enumerate(y1):
                    if y11 == 1.0:
                        y12.append(intent_label_map[idx])
                out_intent_label_list.append(';'.join(y12))

            for y1 in intent_preds:
                intent_preds_list.append({intent_label_map[idx]: p for idx, p in enumerate(y1)})

            texts = ["abc"]*len(intent_preds_list)

            rpt_info, cf_mat_infos, std_pred_cmp_infos, labels = text_multi_label_classification_evaluate(texts, out_intent_label_list, intent_preds_list)

            results.update(rpt_info)
            ...
            for i in range(intent_preds.shape[0]):
                ...
                # if len(set(o1)) == 1:
                #     out_intent_label_list.append("neg_text")
                # else:
                out_intent_label_list.append(intent_label_map[o1.index(max(o1))])

                # if max(p1) < 0.5:
                #     intent_preds_list.append("neg_text")
                # else:
