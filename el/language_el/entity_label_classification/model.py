...
class EntityClassificationModel(BertPreTrainedModel):
    ...
    def __init__(self, model_dir, args):
        ...
        if self.args.is_muti_label:
            self.fc = FCLayer_sigmoid(bert_config.hidden_size*2, self.label_num)
        else:
            self.fc = FCLayer_softmax(bert_config.hidden_size*2, self.label_num)
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, e1_mask, label=None):
