...
def convert_examples_to_features(examples, tokenizer, label_lst, max_seq_length, is_training=True, allow_impossible=True,
                                 pad_sign=True):
...
def read_mrc_ner_examples(input_file, is_training=True, with_negative=True):
    ...
    examples = []
    for ind, entry in enumerate(input_data):
        qas_id = entry["qas_id"]
