...
def kp20k_loader(mode, source_dataset_dir, src_fields=['title', 'abstract'], trg_fields=['keyword'], trg_delimiter=';'):
...
def openkp_refactor(examples, mode):
    ...
    for idx, ex in enumerate(tqdm(examples)):
        doc_words, word2block, block_features = prepro_utils.refactor_text_vdom(text=ex['text'], VDOM=ex['VDOM'])
        data = {}
        data['url'] = ex['url']
