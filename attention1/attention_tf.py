...
def position_embedding(inputs, position_size):
    ...
    # a = tf.expand_dims(position_ij, 0)
    position_embed = tf.expand_dims(position_ij, 0) + tf.zeros((batch_size, seq_len, position_size))
...
if __name__ == '__main__':
    ...
    inputs = tf.random_normal([2, 10, 5])
    print(sess.run(inputs))
    position_embed = position_embedding(inputs, 4)
    print(sess.run(position_embed))
