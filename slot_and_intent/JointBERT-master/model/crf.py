...
class CRF(nn.Module):
    ...
    def decode(self,
               emissions: Union[Tensor, Tensor],
               mask: Optional[Union[Tensor, Tensor]] = None) -> List[List[int]]:
        """Find the most likely tag sequence using Viterbi algorithm.
        Arguments
        ---------
        emissions : :class:`~torch.autograd.tensor` or :class:`~torch.FloatTensor`
            Emission score tensor of size ``(seq_length, batch_size, num_tags)``.
        mask : :class:`~torch.autograd.tensor` or :class:`torch.ByteTensor`
        ...
        if isinstance(emissions, Tensor):
    ...
    def _viterbi_decode(self, emission: Tensor) -> List[int]:
    ...
    @staticmethod
    def _log_sum_exp(tensor: Tensor, dim: int) -> Tensor:
    ...
    def _new(self, *args, **kwargs) -> Tensor:
